from pkrcomponents.components.cards.bitcard import BitCard
...
class LookupTable:
    ...
    def _flushes(self):
        ...
        rank = 1
        for straight_flush in straight_flushes:
            prime_product = BitCard.prime_product_from_rankbits(straight_flush)
        ...
        for flush in flushes:
            prime_product = BitCard.prime_product_from_rankbits(flush)
            self.flush_lookup[prime_product] = rank
            rank += 1
    ...
    def _straight_and_highcards(self, straights, highcards):
        ...
        for straight in straights:
            prime_product = BitCard.prime_product_from_rankbits(straight)
            self.unsuited_lookup[prime_product] = rank
            rank += 1

        rank = LookupTable.MAX_PAIR + 1
        for high_card in highcards:
            prime_product = BitCard.prime_product_from_rankbits(high_card)
            self.unsuited_lookup[prime_product] = rank
            rank += 1
    ...
    def _multiples(self):
        ...
        """
        backwards_ranks = range(len(BitCard.int_ranks) - 1, -1, -1)
        ...
        for i in backwards_ranks:
            ...
            kickers.remove(i)
            for k in kickers:
                product = BitCard.primes[i] ** 4 * BitCard.primes[k]
        ...
        for i in backwards_ranks:
            ...
            for pair_rank in pair_ranks:
                product = BitCard.primes[i] ** 3 * BitCard.primes[pair_rank] ** 2
                self.unsuited_lookup[product] = rank
                rank += 1
        ...
        for b_rank in backwards_ranks:
            ...
            for kickers in itertools.combinations(kickers, 2):
                card1, card2 = kickers
                product = (
                    BitCard.primes[b_rank] ** 3 * BitCard.primes[card1] * BitCard.primes[card2]
                )
                self.unsuited_lookup[product] = rank
                rank += 1
        ...
        for two_pair in itertools.combinations(backwards_ranks, 2):
            ...
            for kicker in kickers:
                product = (
                    BitCard.primes[pair1] ** 2
                    * BitCard.primes[pair2] ** 2
                    * BitCard.primes[kicker]
                )
                self.unsuited_lookup[product] = rank
                rank += 1
        ...
        for pair_rank in backwards_ranks:
            ...
            for kickers in itertools.combinations(kickers, 3):
                ...
                product = (
                    BitCard.primes[pair_rank] ** 2
                    * BitCard.primes[kicker1]
                    * BitCard.primes[kicker2]
                    * BitCard.primes[kicker3]
                )
                self.unsuited_lookup[product] = rank
                rank += 1
